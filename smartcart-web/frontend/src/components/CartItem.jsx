...
export default function CartItem({ item, onRemove, onQuantityChange, loading = false }) {
  ...
  return (
    <>
      <div className={`cart-item ${loading ? 'cart-item--loading' : ''}`} id={`cart-item-${item.id}`} onClick={() => setShowModal(true)}>
        ...
        {/* Loading overlay */}
        {loading && (
          <div className="cart-item__loading-overlay">
            <div className="cart-item__spinner"></div>
          </div>
        )}
      </div>
